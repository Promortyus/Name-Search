...
PACKAGED_CHARACTER_DB_PATH = APP_DIR / "characters.csv"
DATA_DIR = Path(os.environ["NAME_SEARCH_DATA_DIR"]).expanduser() if os.environ.get("NAME_SEARCH_DATA_DIR") else APP_DIR
CHARACTER_DB_PATH = Path(os.environ.get("NAME_SEARCH_CHARACTER_DB_PATH", DATA_DIR / "characters.csv")).expanduser()
...
def ensure_local_character_db_file() -> None:
    if CHARACTER_DB_PATH.exists():
        return

    CHARACTER_DB_PATH.parent.mkdir(parents=True, exist_ok=True)
    if CHARACTER_DB_PATH != PACKAGED_CHARACTER_DB_PATH and PACKAGED_CHARACTER_DB_PATH.exists():
        shutil.copyfile(PACKAGED_CHARACTER_DB_PATH, CHARACTER_DB_PATH)
...
def load_local_character_db() -> pd.DataFrame:
    ensure_local_character_db_file()
...
def find_codex_path() -> str:
    candidates = [
        os.environ.get("NAME_SEARCH_CODEX_PATH", ""),
    ...
    for candidate in candidates:
        if candidate and os.access(Path(candidate).expanduser(), os.X_OK):
