...
- `姓名学取名工具独立版.app`：推荐给日常使用。代码、默认字库、Python Framework 和 Python 依赖都在 app 包里，移动项目文件夹后仍可运行。它会在 app 窗口里显示页面，不再打开浏览器。
...
独立版日志在 `~/Library/Application Support/姓名学取名工具/desktop.log`。
